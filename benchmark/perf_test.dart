...
class _RunBarePerfTest {
  bool _warmup;

  _RunBarePerfTest(this._warmup);
  ...
  call() {
    ...
    jsToDartCompletionCallback(elapsedMilliSeconds) {
      ...
      report("bare messaging performance, javascript call to dart = $usPerCall ns/call");
    ...
    bridgeCompletionCallback(elapsedMilliSeconds) {
      var usPerCall = (1.0e6 * elapsedMilliSeconds) ~/ callCount;
      report("js_bridge messaging performance, javascript call to dart = $usPerCall ns/call");
    }
    ;
    ...
    void dartToJsPerfTest(_) {
      ...
      report("bare messaging performance, dart call to javascript = $perCall $units");
    }
    ;
...
setupPerfTest() {
  test("js_bridge perf warmup", new _RunBarePerfTest(true));
  test("js_bridge perf measure", new _RunBarePerfTest(false));
}
